...
class GenSolver(DetSolver):
    ...
    def gen(self):
        self.setup()
        dataloader = self.cfg.train_dataloader
        # shuffle을 False로 설정하여 새 DataLoader 객체 생성
        dataloader1 = DataLoader(dataloader.dataset, batch_size=4, shuffle=False, num_workers=dataloader.num_workers, pin_memory=dataloader.pin_memory, collate_fn=dataloader.collate_fn)

        model = self.ema.module if self.ema else self.model

        criterion, device = self.criterion, self.device
        model.eval()
        criterion.eval()

        for samples, targets in dataloader1:
            samples = samples.to(device)
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]

            original_index = [int(i['image_id']) for i in targets]
            print(original_index)
            with saving(original_index):
